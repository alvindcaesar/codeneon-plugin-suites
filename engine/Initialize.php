...
namespace Codeneon_Plugin_Suites\Engine;

use Codeneon_Plugin_Suites\Engine;
...
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'codeneon_plugin_suites_classes_to_execute', $this->classes );
		...
		foreach ( $this->classes as $class ) {
			try {
				...
				\do_action( 'codeneon_plugin_suites_initialize_failed', $err );
	...
	private function get_classes( string $namespace ) {
		...
		$namespace = 'Codeneon_Plugin_Suites\\' . $namespace;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Codeneon_Plugin_Suites\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespace ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Plugin Name is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', CPS_TEXTDOMAIN ) );
			}
