...
class ActDeact extends Base {
	...
	public function initialize() {
		...
		\register_activation_hook( CPS_TEXTDOMAIN . '/' . CPS_TEXTDOMAIN . '.php', array( self::class, 'activate' ) );
		\register_deactivation_hook( CPS_TEXTDOMAIN . '/' . CPS_TEXTDOMAIN . '.php', array( self::class, 'deactivate' ) );
	...
	public static function upgrade_procedure() {
		...
		$version = \get_option( 'codeneon-plugin-suites-version' );

		if ( !\version_compare( CPS_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'codeneon-plugin-suites-version', CPS_VERSION );
		\delete_option( CPS_TEXTDOMAIN . '_fake-meta' );
	}
